...
PAT_WRAP_KEY_BASE64=<32-byte-base64>
PAT_WRAP_KEY_VERSION=v1
MAX_SEAL_JSON_BODY_BYTES=32768
MAX_GITHUB_PROXY_BODY_BYTES=8388608

# 必填（跨域）
CORS_ORIGINS=https://foxmaybeoi1761640545.github.io,http://localhost:5173,http://127.0.0.1:5173,http://localhost:4173,http://127.0.0.1:4173
...
1. CORS_ORIGINS 只填 origin，不要带路径（不要 /car-expense-journal-.../）。
2. PORT 不用填，Render 会自动注入。
3. 保存变量后要触发一次重新部署。
4. PAT_WRAP_KEY_BASE64 泄露就立刻更换，并让用户重新保存一次 PAT。
...
TICK_MAX_ENTITY_FILES=350
TICK_MAX_DUE_REMINDERS=35
TICK_DUE_LOOKBACK_SECONDS=604800
TICK_PUSH_TTL_SECONDS=600
MAX_PUSH_JSON_BODY_BYTES=131072
MAX_REMINDER_JSON_BODY_BYTES=131072
MAX_GITHUB_PROXY_BODY_BYTES=8388608
MAX_SEAL_JSON_BODY_BYTES=32768
ENABLE_PORT_FALLBACK=true
MAX_PORT_FALLBACK_STEPS=20
```
...
OPENAI_TRIP_IMAGE_MODEL=gpt-5.3-codex
OPENAI_API_BASE_URL=https://api.openai.com/v1
TRIP_IMAGE_UPLOAD_DIR=uploads/trip-images
MAX_TRIP_AI_JSON_BODY_BYTES=0
```
...
OPENAI_API_KEY=<your-openai-api-key>
OPENAI_TRIP_IMAGE_MODEL=gpt-5.3-codex
OPENAI_API_BASE_URL=https://cli-config-test-1774617155.onrender.com/v1
TRIP_IMAGE_UPLOAD_DIR=uploads/trip-images
MAX_TRIP_AI_JSON_BODY_BYTES=0

# 新增：codex-cli fallback
ENABLE_CODEX_CLI_FALLBACK=true
TRIP_AI_CODEX_CLI_BIN=codex
TRIP_AI_CODEX_CLI_MODEL=gpt-5.3-codex
TRIP_AI_CODEX_CLI_TIMEOUT_MS=180000

# 新增：让 codex 从项目根目录读取 auth.json
CODEX_HOME=/opt/render/project/src/.codex
```
